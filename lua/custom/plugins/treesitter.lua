return {
...

  {
    ...
    config = function(_, opts)
      ...
      local function attach(buf, opts)
        local ft = vim.bo[buf].filetype

        -- 1. Check if Treesitter parser exists for this filetype
        ...
        -- 2. Check if movement is enabled
        if not (opts.move and opts.move.enable) then
          return
        end

        -- 3. Get move keys (like goto_next_start, goto_prev_start)
        ...
        for method, keymaps in pairs(moves) do
          for key, query in pairs(keymaps) do
            ...
            -- Avoid conflict with diff mode
            if not (vim.wo[buf].diff and key:find '[cC]') then
